...
const CallList = ({ type }: { type: 'ended' | 'upcoming' | 'recordings' }) => {
  ...
  if (calls && calls.length > 0) return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-3"> {/* Grid layout for calls */}
      {
        calls.map((meeting: Call | CallRecording, index) => { // Map over calls
          const d = (meeting as Call).state?.startsAt?.toLocaleString('fa-IR') || // Use start time if available
            new Date((meeting as CallRecording).start_time).toLocaleString('fa-IR') // Use recording start time if available

          return (
            <MeetingCard
              call={meeting as Call} // Cast meeting as Call
              key={index} // Use call ID as key
  ...
  return (
    <Alert
      title='هیچ نشستی وجود ندارد' // Alert title
